...
void setup() {
  ...
  Serial.println();
  Serial.print("Select a type of conversion to perform the ADC ('a' for ");
  Serial.println("built-in analog read function, 'b' for register config for polling and port manipulation, or 'c' for register config for interrupts)");      
}
void loop() {
  String userInput;
...
long optionC(long prevTime, int analogVal) {
    ...
    ADCSRA |= _BV(ADIE) | _BV (ADPS2) | _BV (ADPS1) | _BV (ADPS0);
      ...
      for(int x=0; x<25; x++){
          ...
          long convTime = micros();//initialize convTime for each iteration
          //(ADEN=1)(ADSC=1)(ADIF=0)(ADIE=1)(ADPS2=1)(ADPS1=1)(ADPS0=0)
          //conv starts if ADSC set(ADCSRA == B11001111)
          while(ADCSRA & (1<<ADSC)){
            //waits for interrupt flag (ADIF==1) 
          }
          convTime = (micros()-convTime);//subtract starting time
          analogVal = adc;//store in local variable analogVal
